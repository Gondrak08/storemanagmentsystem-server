        ...
        if (!err) {
            ejs.renderFile(path.join(__dirname, '', "report.ejs"), { productDetails: productDetailsReport, name: orderDetails.name, email: orderDetails.email, contactNumber: orderDetails.contactNumber, paymentMethod: orderDetails.paymentMethod, totalAmount: orderDetails.totalAmount }, (err, results) => {
                if (err)
                    ...
                    return pdf.create(results).toFile('./generated/' + generateUuid + ".pdf", function(err,data) {
                        if (err) {
                            return console.log("Hellooo"), res.status(500).json(err)
                        } else {
            ...
            return res.status(500).json(err), console.log(err, "bill error") ;
        }
    })
...
router.post('/getPdf', auth.authenticateToken, function (req, res) {
    const orderDetails = req.body;
    const pdfPath = './generated/' + orderDetails.uuid + '.pdf';
    if (fs.existsSync(pdfPath)) {
        ...
        ejs.renderFile(path.join(__dirname, '', "report.ejs"), { productDetails: productDetailsReport, name: orderDetails.name, email: orderDetails.email, contactNumber: orderDetails.contactNumber, paymentMethod: orderDetails.paymentMethod, totalAmount: orderDetails.totalAmount }, (err, results) => {
            if (err)
                ...
                return pdf.create(results).toFile('./generated/' + orderDetails.uuid + ".pdf", function (err, data) {
                    if (err) {
                        console.log(err);
